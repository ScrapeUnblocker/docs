...
<Update label="2026-09-24" description="Google Images: up to 5 result pages, market follows proxy_country">
  [`POST /images/google-search`](/plugins/google-images) takes a new **`pages`**
  parameter (`1`-`5`, ~100 results per page, Google stops at roughly 400-500 per
  query). Each page fetched is billed as one request; the response reports it in
  **`pagesFetched`**. The Google market now **follows `proxy_country`**
  automatically - `gl` is an optional override and empty by default (before, it
  defaulted to `us` even when another `proxy_country` was set). `max_results` is
  now an optional cap up to `500`. One-page calls behave exactly as before.
</Update>

<Update label="2026-09-23" description="Dead URLs: X-Origin-Status, billing and a complete error reference">
  When the target site answers that a page **does not exist** (`404` / `410`),
  `getPageSource` now always returns it as a `200` with the page as served and
  an **`X-Origin-Status`** header carrying the target's status - the same
  header a dead origin's own `5xx` page already had. Previously some of these
  came back as `403 Blocked by bot protection.`, which read as a block. These
  calls are billed like any delivered page; a URL whose domain does not exist
  returns `400 Hostname cannot be resolved` and is billed too. Responses our
  load balancer generates itself (`502` / `503` / `504`) are never billed.
  [Errors](/errors) now lists every status with whether it is billed, and
  explains how to tell a target problem from an outage on our side;
  [Handling failures](/guides/handling-failures) covers `503`, `504`, `523` and
  dead URLs.
</Update>

<Update label="2026-09-22" description="Mobile.de plugin">
  **New plugin page:** [`POST /vehicles/mobile-de`](/plugins/mobile-de). Search
  mobile.de, Germany's largest used-car marketplace, by make and get clean,
  parsed JSON with pagination to `max_results`: title, make and model, price
  (EUR), first registration, mileage, power (kW/PS), fuel and a direct listing
  URL. Filter by `price_min`/`price_max`, `year_min`, `mileage_max` and `fuel`;
  pass a friendly `make` name or a raw `make_id`. It runs on the same anti-bot
  fetch chain as `getPageSource` - no per-site API key, account or CAPTCHA setup.
</Update>

<Update label="2026-09-22" description="Four marketplace plugins: Target, Vinted, Craigslist and Zalando">
  **New plugin pages:** [`POST /marketplace/target-search`](/plugins/target),
  [`POST /marketplace/vinted-search`](/plugins/vinted),
  [`POST /marketplace/craigslist-search`](/plugins/craigslist) and
  [`POST /marketplace/zalando-search`](/plugins/zalando). Each returns clean,
  parsed JSON with pagination to `max_results`: **Target** US retail search
  (tcin, title, brand, price, image, URL) via Target's own Redsky API;
  **Vinted** EU resale listings on any regional marketplace (title, price and
  currency, brand, size, condition, image); **Craigslist** US classifieds in any
  metro (posting id, title, price, location, image); and **Zalando** EU fashion
  search on any storefront (sku, brand, name, price and currency, image). All run
  on the same anti-bot fetch chain as `getPageSource` - no per-site API key,
  account or CAPTCHA setup.
</Update>

<Update label="2026-09-22" description="Google Images: googleImagesUrl (/imgres viewer link) per result">
  [`POST /images/google-search`](/plugins/google-images) now returns a
  **`googleImagesUrl`** on every result - the Google Images viewer link
  (the `/imgres?...` URL a browser copies from a result tile), reconstructed
  from the result data. Existing fields are unchanged; this is purely additive.
</Update>

<Update label="2026-09-22" description="Five marketplace, travel and jobs plugins documented">
  **New plugin pages:** [`POST /marketplace/walmart-search`](/plugins/walmart),
  [`POST /marketplace/etsy-search`](/plugins/etsy),
  [`POST /travel/booking`](/plugins/booking),
  [`POST /travel/airbnb`](/plugins/airbnb) and
  [`POST /jobs/indeed`](/plugins/indeed). Each returns clean, parsed JSON with
  pagination to `max_results`: **Walmart** and **Etsy** product search (title,
  price, rating, shop/seller, image, URL); **Booking.com** hotels (name, price
  for your dates, review score, address, coordinates); **Airbnb** stays (name,
  room type, nightly price, rating, coordinates, badges); and **Indeed** jobs
  (title, company, parsed salary min/max, job types, company rating, snippet).
  All run on the same anti-bot fetch chain as `getPageSource` - no per-site API
  key, account or CAPTCHA setup.
</Update>

<Update label="2026-09-21" description="Car Hire: target a location by ID + clearer resolution">
  **[`POST /carhire/skyscanner-quotes`](/plugins/skyscanner#car-hire) now takes
  `pickup_entity` / `dropoff_entity`** - Skyscanner's own location IDs. Pass one
  to skip the name lookup and search that exact place, useful for a town the
  autosuggest can't reach or a name that resolves ambiguously. A place **name**
  that can't be resolved now returns an **error instead of silently searching a
  different location**, and the `X-Pickup-Entity` / `X-Pickup-Name` (and
  `X-Dropoff-*`) response headers report the location that was actually searched.
</Update>

<Update label="2026-09-17" description="Google Images plugin">
  **New plugin: [`POST /images/google-search`](/plugins/google-images).** Google
  Images results for a keyword as clean JSON - the **full-resolution original
  image URL** and **source domain** for every result, plus the source page URL,
  title, site name, thumbnail and dimensions. The current Google Images page
  carries every result's origin image URL inline, so `imageUrl` is the real
  image on the publisher's site, not a Google thumbnail. Runs on the same warm,
  anti-bot-cleared Google sessions as our SERP endpoint. Pass `q`, and
  optionally `proxy_country` / `gl` / `max_results` (1-100).
</Update>

<Update label="2026-09-17" description="HTTP 523 for an unreachable origin (not a block)">
  **`getPageSource` now answers `523` when the target origin itself is
  unreachable** - down, refusing connections, or flapping (brief outages, then
  fine) - instead of a `403 Blocked by bot protection.`. A 523 is not a bot
  block and is never billed; it means the site did not answer over any of our
  exits, so the right move is to retry later. `403` stays reserved for a real
  anti-bot block we could not clear. If your client keyed retries on `403`
  only, treat `523` the same way (retry after a short pause).
</Update>

<Update label="2026-09-16" description="Southwest Airlines fares plugin">
  **New plugin: [`POST /flights/southwest-quotes`](/plugins/southwest).** Live
  Southwest Airlines fares as clean JSON - dollar and Rapid Rewards points
  prices for every flight on a route, one-way or return. Southwest sells no
  public fare API and fronts its site with Akamai Bot Manager; the plugin runs
  the search on a warm, anti-bot-cleared session and returns Southwest's own
  `booking/shopping` payload (fare products Wanna Get Away / Wanna Get Away Plus
  / Anytime / Business Select, with taxes and Rapid Rewards points). Origin and
  destination are IATA airport codes; `fare_type` is `dollars` or `points`.
</Update>

<Update label="2026-09-10" description="YouTube channel endpoint returns exact counts, duration and channel stats">
  **`POST /content/youtube-channel` now returns richer data.** The endpoint
  reads a channel's uploads through the YouTube Data API, so each video now
  carries exact `viewCount`, `likeCount` and `commentCount`, `duration`
  (ISO-8601 and `durationSeconds`), `description` and a `publishedAt`
  timestamp, alongside the existing `views` / `published` text. The `channel`
  summary gains `subscriberCount`, `videoCount`, total `viewCount` and the
  handle. A new `limit` parameter (1-200) pages past the first batch of videos.
  If the Data API is unavailable the endpoint falls back to the previous scrape,
  so responses stay compatible.
</Update>

<Update label="2026-09-09" description="getPageSource redirects Temu search and product URLs to the plugins">
  **[`getPageSource`](/guides/page-source) now redirects Temu search and product
  URLs.** A `POST /getPageSource` for a Temu search page (`/search_result.html`)
  or a product page (a `-g-<id>.html` URL) returns `400` with the endpoint that
  actually works - the [Temu plugin](/plugins/temu) (`/goods/temu-search`,
  `/goods/temu-product`) - in the body and the `X-Recommended-Endpoint` header.
  Those pages hydrate client-side and the generic browser path renders them
  slowly and hands back an empty shell, while the plugin reads Temu's `ld+json`
  directly. Every other Temu page (homepage, category pages) still works through
  `getPageSource`.
</Update>

<Update label="2026-09-08" description="New TikTok plugin - creators, videos and hashtags as JSON">
  **[TikTok plugin](/plugins/tiktok).** Three new endpoints read public TikTok
  data with no login: `POST /social/tiktok-profile` (exact follower / like /
  video counts, bio, link, flags and the creator's newest videos, up to 200),
  `POST /social/tiktok-video` (one video or photo post - exact plays, likes,
  comments, shares, saves and reposts, hashtags, mentions, author, music,
  play / download / per-quality URLs, subtitle tracks and, with
  `include_transcript=true`, the transcript as text) and `POST
  /social/tiktok-hashtag` (total views and videos under a tag plus its videos,
  up to 200), plus `POST /social/tiktok-search` (the videos TikTok ranks for a
  query, region selectable) and `POST /social/tiktok-comments` (a post's
  comments with author, likes and replies) - both run in a browser session
  that solves TikTok's captcha on the fly. Measured over 750 production calls
  for the first three: 99.3% success, 5.9 s average (single videos about 2 s).
</Update>
