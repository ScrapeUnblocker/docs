...
<Update label="2026-07-16" description="Python SDK">
  Released the official **Python client** on PyPI:

  ```bash
  pip install scrapeunblocker
  ```

  Sync `Client` and async `AsyncClient` wrapping `getPageSource` (HTML / parsed JSON / cookies), `serpApi`, `getImage`, and the Skyscanner plugins - with typed errors and automatic retries. See the [Python guide](/sdks/python).
</Update>
