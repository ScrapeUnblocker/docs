...
<Update label="2026-08-13" description="Temu search now returns full products with prices">
  **[Temu search](/plugins/temu#search) upgraded.** `POST /goods/temu-search`
  now returns about **40 products per keyword with their fields already filled
  in** - `name`, `price`, `marketPrice` (RRP), `currency`, `rating`,
  `reviewCount`, `sold`, `image` and `url` - instead of bare URLs. No
  `details=true` round-trip needed. Typical response time ~2s.
</Update>
