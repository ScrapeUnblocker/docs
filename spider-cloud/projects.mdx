...
If your `settings.py` builds its configuration from a settings blob handed to it by whatever ran it, rather than from `os.environ`, then it **never reads the environment**. Moved somewhere new, such a module quietly falls back to its hardcoded defaults, and anything you put in the environment is never read by anyone.
