...
curl -X POST "https://api.scrapeunblocker.com/getPageSource" \
  ...
  --data-urlencode 'steps=[{"action":"type","selector":"input[name=q]","value":"headphones"},{"action":"press_key","value":"Enter"},{"action":"wait_for","selector":".results"}]'
```

Full action reference, limits and the `422` failure shape are in
