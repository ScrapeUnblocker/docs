...
`resultsCollected`), `pagesFetched` (how many result pages were rendered - this
is how many API requests the call was billed, see [`pages_to_check`](#pages-to-check)),
the three result lists (`topAds`, `bottomAds`, `organic`) and `aiOverview`. On a country fallback it also carries `requestedCountry` and
...
**Billing:** each result page rendered is charged as one API request - a `pages_to_check=3` call that returns three pages counts as 3 requests, not 1. The `pagesFetched` field in the response tells you exactly how many were charged (it can be lower than `pages_to_check` when Google has fewer pages). This per-page charge is unique to `/serpApi`; every other endpoint bills a flat 1 per request regardless of how many results it returns.
