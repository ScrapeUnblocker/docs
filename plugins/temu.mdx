...
<Note>
Calling `POST /getPageSource` on a Temu **search** (`/search_result.html`) or
**product** (`-g-<id>.html`) URL returns `400` and points you here - those pages
hydrate client-side and don't render through the generic browser path. Every
other Temu page still works through `getPageSource`.
</Note>
