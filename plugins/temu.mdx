...
Search Temu for a keyword and get back a page of products with their prices,
ratings and images already filled in - straight from Temu's own search page,
about 40 products per call.
...
| `limit` | no | Max products to return, default `40`, max `100`. One search page carries ~40. |
| `details` | no | Rarely needed. Products already come with full fields; this only fills in any that arrived as URL-only, by resolving them to the product document. Default `false`. |
| `details_limit` | no | How many URL-only results to resolve when `details=true`. Default `8`, max `20`. |
...
```json
{
  ...
  "products": [
    {
      "goodsId": "601099613072998",
      "name": "Kids Educational Building Blocks Set, 253pcs",
      "price": 11.68,
      "marketPrice": 14.77,
      "currency": "EUR",
      "rating": 4.8,
      "reviewCount": "8 914",
      "sold": "87K+",
      "image": "https://img.kwcdn.com/product/fancy/xxxx.jpg",
      "url": "https://www.temu.com/lt-en/kids-toy-g-601099613072998.html"
    }
  ]
}
```

Fields per product: `goodsId`, `name`, `price` (a number, e.g. `11.68`),
`marketPrice` (the crossed-out reference price, `null` when there is no
discount), `currency`, `rating`, `reviewCount`, `sold`, `image` and `url`. A
field is `null` when Temu did not supply it for that listing.
...
- **Competitive research** - pull a keyword's top ~40 listings and compare prices, RRPs and ratings.
