...
Search Temu for a keyword and get back product URLs, taken from Temu's own
search page. With `details=true` it also resolves the top results to full product
objects (same shape as **Product** above).
...
| `limit` | no | Max product URLs to return, default `40`, max `100`. Temu serves ~14 per search page. |
| `details` | no | If `true`, also resolve the top `details_limit` URLs to full product objects. Each is an extra fetch and counts toward the rate limit. Default `false`. |
| `details_limit` | no | How many results to resolve when `details=true`. Default `8`, max `20`. |

```bash cURL
# URLs only
curl -X POST "https://api.scrapeunblocker.com/goods/temu-search?keyword=kids%20toys&limit=20" \
  -H "x-scrapeunblocker-key: YOUR_API_KEY"

# URLs plus full product objects for the top 8
curl -X POST "https://api.scrapeunblocker.com/goods/temu-search?keyword=kids%20toys&details=true&details_limit=8" \
  -H "x-scrapeunblocker-key: YOUR_API_KEY"
```
...
```json
{
  ...
  "resultsCollected": 14,
  "urls": [
    "https://www.temu.com/kids-toy-g-601099500000001.html",
    "https://www.temu.com/other-toy-g-601099500000002.html"
  ],
  "products": [
    {
      "url": "https://www.temu.com/kids-toy-g-601099500000001.html",
      "name": "...",
      "price": "10.64",
      "priceCurrency": "EUR",
      "availability": "https://schema.org/InStock",
...
`products` is present only when `details=true`. If the rate budget runs out
mid-resolve, `products` carries the ones fetched so far and `urls` still lists
them all.
...
- **Competitive research** - pull a keyword's top listings with `details=true` and compare prices and ratings.
