...
| `proxy_country` | no | Country to search from (ISO-2, e.g. `US`, `DE`). The Google market (which images rank) follows it automatically. Without it you get US-market results. |
| `pages` | no | How many result pages to fetch, `1`-`5`. Each page has ~100 results; Google stops at roughly 400-500 per query. **Each page fetched is billed as one request** (see `pagesFetched`). Default `1`. |
| `gl` | no | Optional Google market override (ISO-2 lowercase, e.g. `de`). Leave it empty to follow `proxy_country`; set it only for a different market than the IP country. |
| `max_results` | no | Optional cap on the number of results returned, `1`-`500`. Default: everything the fetched pages return. |

```bash
curl -X POST "https://api.scrapeunblocker.com/images/google-search?q=eiffel%20tower&proxy_country=US&pages=3" \
  ...
  "results": [
    {
      ...
      "height": 1067,
      "fileSize": "418KB",
      "googleImagesUrl": "https://www.google.com/imgres?q=eiffel%20tower&imgurl=https%3A%2F%2Fcdn.britannica.com%2F31%2F255531-050-B7E07090%2Feiffel-tower-paris-france.jpg&imgrefurl=https%3A%2F%2Fwww.britannica.com%2Ftopic%2FEiffel-Tower-Paris-France&docid=kZ9Xr2mFqL8AbM&tbnid=Xy7Qm2ntAbCDeM&w=1600&h=1067&hcb=2&ved=2ahUKEw..."
    }
  ]
}
```
...
| `googleImagesUrl` | The Google Images tile link (`/imgres?...`) for this result - the same format a desktop browser copies from the result tile ("Copy link address"), rebuilt from the tile's own ids: `q`, `imgurl`, `imgrefurl`, `docid`, `tbnid`, `w`, `h`, `hcb` and `ved`. The identifiers (`docid`, `tbnid`, `imgurl`, `imgrefurl`) match Google's exactly; `ved` is a per-search-session token (Google mints a fresh one each search, so it never matches across sessions). Returned for every result. |
